...
const ROLE_NAME_STORAGE_KEY = 'sams-copy-role-name';
const ACCESS_LEVEL_STORAGE_KEY = 'sams-copy-access-level';
...
export default function AddUserRole() {
  ...
  const [roleName, setRoleName] = useState(() => {
    ...
    return window.localStorage.getItem(ROLE_NAME_STORAGE_KEY) ?? '';
  });
  const [accessLevel, setAccessLevel] = useState(() => {
    ...
    return window.localStorage.getItem(ACCESS_LEVEL_STORAGE_KEY) ?? '';
  });
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(ROLE_NAME_STORAGE_KEY, roleName);
  }, [roleName]);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(ACCESS_LEVEL_STORAGE_KEY, accessLevel);
  }, [accessLevel]);
  ...
  const handleCreateNewRole = () => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.removeItem(ROLE_NAME_STORAGE_KEY);
      window.localStorage.removeItem(ACCESS_LEVEL_STORAGE_KEY);
    }
  };
